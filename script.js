...
// ── JSONBIN CONFIG ── ganti dengan BIN ID dan API KEY kamu
...
// Ambil data mod dari JSONBin saat halaman dibuka
async function loadMods() {
  try {
    ...
    MODS = Array.isArray(data.record) ? data.record : [];
  } catch(e) {
    MODS = [];
  }
  renderGrid();
...
// Ambil data mod dari JSONBin saat halaman dibuka
const MODS_DEFAULT = [...MODS]; // simpan data default dari script.js
...
async function loadMods() {
  try {
    ...
    const data = await res.json();
    const fromBin = Array.isArray(data.record) ? data.record : [];
    // kalau JSONBin ada data, pakai dari sana. Kalau kosong, pakai data di script.js
    MODS = fromBin.length > 0 ? fromBin : MODS_DEFAULT;
  } catch(e) {
    // kalau error / offline, tetap pakai data di script.js
    MODS = MODS_DEFAULT;
  }
  renderGrid();
  updateStats();
